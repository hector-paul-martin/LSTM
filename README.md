...
The LSTM is mathematically standard in the forward and backward pass. It uses forget, candidate, input and output gates.
...
Starting on small sequence lengths works better because after the network 'loses track' of the parity its outputs beyond that point lose any relation to the labels. This means gradients from beyond the point of first failure become noise, so training on small sequence lengths and scaling up to keep any failure points close to the end of the sequence minimises the gradients that become noise, making training more efficient. This method allows the network to achieve 100% accuracy on a sequence length of 1000 across 10,000 test examples.
